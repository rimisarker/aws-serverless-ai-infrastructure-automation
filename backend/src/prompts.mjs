...
export function titleFor(action, payload) {
  // Fixed logic: If payload already contains a complete title (like Upload: filename), use it directly
  if (payload.title) return sanitizeText(payload.title).slice(0, 100);
  ...
  const label = {
    ...
    progress: 'Progress',
    upload: 'Upload' // Fallback label for safety
  }[action] || 'AI Response';
